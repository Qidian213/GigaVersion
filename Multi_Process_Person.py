...
def sub_processor(pid, result_dict, sub_file_list):
    for idx, imgname in enumerate(sub_file_list):
        ...
        for clu in range(n_clusters):
            ...
            if((right-left)>4096 or (bottom-top)>4096):
               ...
               # for left_p, top_p, right_p, bottom_p in zip([0, width_p//2], [0,height_p//2], [width_p//2, width_p], [height_p//2,height_p]):
                ...
                while left_p < width_p:
                    if left_p + 2048 >= width_p:
                        left_p = max(width_p - 2048, 0)
                        
                    top_p = 0
                    while top_p < height_p:
                        if top_p + 2048 >= height_p:
                            top_p = max(height_p - 2048, 0)
                            
                        right_p  = min(left_p + 2048, width_p - 1)
                        bottom_p = min(top_p + 2048, height_p - 1)
                        ...
                        ious_boxs_p   = select_boxs[ious>0]
                        ious          = ious[ious>0]
                        ingore_boxs_p = ious_boxs_p[ious<cutiou]
                        ...
                        ingore_boxs_p[:, 0] -= left_p
                        ingore_boxs_p[:, 1] -= top_p
                        ingore_boxs_p[:, 2] -= left_p
                        ingore_boxs_p[:, 3] -= top_p
                        
                        for box in ingore_boxs_p:   
                            subimg_p[int(box[1]): int(box[3]), int(box[0]):int(box[2])] = np.array([125,125,125])
                        ...
                        result_dict[save_name] = {'width': su_width, 'height': su_height, 'bboxs': select_boxs_p.tolist()}
...
if __name__ == '__main__':
    ...
    outpath     = '/data/Dataset/GigaVersion/image_full_split_k_means_20/image_train/'
    outannofile = '/data/Dataset/GigaVersion/image_full_split_k_means_20/panda_full.json'
